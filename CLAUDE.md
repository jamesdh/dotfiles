...
- Machine-profile Brewfiles: `Brewfile.personal` / `Brewfile.proximal` — selected by `~/.dotfiles-profile` (`personal` or `proximal`, prompted once by bootstrap.sh). Missing/unknown profile installs neither, deliberately. Laptop-vs-desktop gating uses battery presence (`battery_section` in `roles/osx/tasks/settings.yml`), NOT the model id — modern Apple Silicon ids (`Mac16,10`) don't contain "Book".
