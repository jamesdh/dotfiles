...
- **Architecture Tests Enforce Boundaries, Never Implementations.** A boundary is a dependency or visibility rule between components: which packages may depend on which, which types stay package-private, which callers may reach a repository or table. Make boundaries executable — prefer compile-time enforcement (visibility, module structure), use ArchUnit or equivalent for what the compiler cannot express, and add a rule whenever you introduce a new component boundary. Never use an architecture test to pin an implementation choice: which annotation a method carries, which framework API a class uses, how a cache is keyed, what a method is named, how a computation is organized. Litmus test: if a correct, faster, or simpler implementation of the same observable behavior would fail the rule, it is not a boundary rule — put that expectation in an ordinary test that asserts outcomes.
...
- **Package by Component.** A component is a package tree whose only public types are the interfaces and value types in its root package. Every implementation, repository, adapter, cache, scheduler, and helper is package-private, exposed to consumers only as a bean typed by its interface. Consumers depend on interfaces and values; a concrete class that a consumer can name is a defect, however well it is documented. The rule is recursive: each subpackage is a component with the same shape, and neighbors inside the parent depend on each other's interfaces, never their classes. A caller that needs an answer or an action gets a method on the interface, never a widened type. An event is not a substitute for that call: publish one only when something happened inside the component that other components must react to, a push to listeners, never a way around package privacy. Java package-private is the enforcement; ArchUnit covers only what the compiler cannot see, such as a subpackage depending on another subpackage's implementation. Never flatten packages, never move a type to a parent package, and never make a class public to get access. When auditing a boundary, list every public type outside code depends on and ask of each: interface, value, or implementation? Implementation means a leak to fix now.
