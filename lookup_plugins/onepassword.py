...
class OnePass(object):
    ...
    def get_raw(self, item_type, item_id, vault=None):
        args = ["get", item_type, item_id]
    ...
    def get_field(self, item_type, type_opts, item_id, field, section=None, vault=None):
        output = self.get_raw(item_type, item_id, vault)
    ...
    def _parse_field(self, data_json, item_type, type_opts, field_name, section_title=None):
        data = json.loads(data_json)
        if section_title is None:
            if field_name == 'notesPlain':
                return data['details'].get('notesPlain')
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        # item + document support only so far
        obj_type = kwargs.get('type', 'item')
        # text (utf-8) or binary (bytes)
        type_opts = kwargs.get('encoding', 'utf-8')
        section = kwargs.get('section')
        vault = kwargs.get('vault')
        op.subdomain = kwargs.get('subdomain', 'my')
        ...
        values = []
        for term in terms:
            values.append(op.get_field(obj_type, type_opts, term, field, section, vault))
        return values